// this was ben! sorry for recommit!
  ...
  async scraper(browser, start: number, end: number) {
    ...
    async function scrapeUNC() {
      ...
      let getData = async (term, course) => {
        ...
        if (shouldContinue.length > 0) return dataObj;
        ...
        for (let row of rows) {
          ...
          if (!firstTime) {
            ...
            if (columns.length < numFields) {
              if (columns.length < numFields - 1) {
                // No Subject and no Catalog Number
